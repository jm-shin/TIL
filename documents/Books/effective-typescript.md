...
코드를 표현하고자 하는 모든 분야에는 주제를 설명하기 위한 전문 용어들이 있습니다. 자체적으로 용어를 만들어 내려하지 말고, 해당 분양에서 이미 존재하는 용어를 사용해야 합니다. 이러한 용어사용이 사용자와 소통에
유리하며 타입의 명확성을 올린다.
...
- 이름을 지을 때는 포함된 내용이나 계산방식이 아니라 데이터 자체가 무엇인지 고려해야합니다. 에를 들어 INodeList보다 Directory가 더 의미 있는 이름입니다. Directory는 구현의 측면이 아니라
  개념적인 측면에서 디렉터리를 생각하게 합니다.
